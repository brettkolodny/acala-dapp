...
import { Token, TokenPair, FixedPointNumber, getPresetToken, currencyId2Token } from '@acala-network/sdk-core';
import { SwapTrade } from '@acala-network/sdk-swap';
import { Fee, SwapTradeMode } from '@acala-network/sdk-swap/help';

import { useAllTradingPairs, useApi } from '@acala-dapp/react-hooks';
...
export const SwapProvider: FC<PropsWithChildren<{}>> = memo(({ children }) => {
  const { api } = useApi();
  const allTradingPairs = useAllTradingPairs();
  ...
  const enableTokenPairs = useMemo((): TokenPair[] => {
    ...
    return allTradingPairs.map((item) => {
      return new TokenPair(
        currencyId2Token(item[0]),
        currencyId2Token(item[1])
      );
    });
  }, [api, allTradingPairs]);
