...
export const DepositConsole: FC = () => {
  ...
  ] = useInputValue<Partial<BalanceInputValue>>({ amount: 0 });
  ...
  const {
    availableLP: isAvailableLP,
    getAddLPSuggestAmount,
    lpCurrencyId
  } = useLP(token1.token, token2.token);
  ...
  const handleToken1CurrencyChange = useCallback((currency: CurrencyId) => {
    setToken1({ token: currency });
    clearAmount();
  }, [setToken1, clearAmount]);

  const handleToken2CurrencyChange = useCallback((currency: CurrencyId) => {
  ...
  const params = useCallback(() => {
    ...
    return [
      ...
      token2.token,
      eliminateGap(
        new FixedPointNumber(token1.amount),
        token1Balance
      ).toChainData(),
      eliminateGap(
  ...
  return (
    <CardRoot>
      ...
      <Row gutter={[0, 24]}>
        ...
        <Col span={24}>
          <TokenInput
            currencies={token1Currencies}
            onChange={handleToken1CurrencyChange}
            value={token1.token}
          />
        </Col>
        <Col>
        ...
        <Col span={24}>
          <TokenInput
            currencies={token2Currencies}
            onChange={handleToken2CurrencyChange}
            value={token2.token}
          />
        </Col>
        {
