...
export const useIncentivePool = (poolId: PoolId, currency: CurrencyId): CPoolInfo => {
  const { data: poolInfo } = useQuery<PoolInfo>('query.rewards.pools', [getPoolId(poolId, currency)]);
...
export const useShareAndWithdrawnReward = (poolId: PoolId, currency: CurrencyId, account?: string): ShareAndWithdrawn => {
  const { data: info } = useQuery<[Share, Balance]>('query.rewards.shareAndWithdrawnReward', [getPoolId(poolId, currency), account]);
...
export const useDexActiveIncentiveRewardPool = (): RewardPool[] => {
  const { api } = useApi();
  const { data: dexIncentiveRewards } = useQuery<[StorageKey, Balance][]>('query.incentives.dEXIncentiveRewards.entries');
...
export const useLoanActiveIncentiveRewardPool = (): RewardPool[] => {
  const { api } = useApi();
  const { data: loansIncentiveRewards } = useQuery<[StorageKey, Balance][]>('query.incentives.loansIncentiveRewards.entries');
  const result = useMemo(() => {
